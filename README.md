...
- Selectable LR construction algorithm: `%define lr.type {canonical-lr | lalr}`. Default canonical LR(1) for clean diagnostics; opt into LALR(1) when build time or bundle size start to hurt. Per-terminal `%shift` / `%reduce` escape hatches resolve ambiguity in either mode. (Concrete data: `examples/ada_full.uplox` builds to 58611 states under canonical LR(1) and 1594 states under LALR(1) — ~37× shrink, both conflict-free.)
