...
uplox::<grammar>``. Two grammars built with uplox can coexist in the same
binary because their identifiers are reachable only through different
namespaces.

Generated outputs:

* ``uplox_<grammar>.hpp`` — public types + Parser class declaration.
* ``uplox_<grammar>.cpp`` — embedded tables, scanner, parser implementation.
