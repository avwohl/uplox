...
A uplox bundle is a single JSON object with a fixed top-level shape:
...
def empty_bundle(grammar_name: str) -> dict[str, Any]:
    ...
    """
    return {
        "uplox_schema": UPLOX_SCHEMA_VERSION,
...
def dump_bundle(bundle: dict[str, Any]) -> str:
    ...
    if bundle.get("uplox_schema") != UPLOX_SCHEMA_VERSION:
        raise ValueError(
            f"bundle.uplox_schema must be {UPLOX_SCHEMA_VERSION!r}, got {bundle.get('uplox_schema')!r}"
        )
