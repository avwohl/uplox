...
The canonical LR(1) builder in :mod:`uplox.parse.lr1` already records every
...
* :mod:`uplox.parse.glr.table` — a :class:`GLRTable` view that derives a
  multi-action ACTION map from an :class:`~uplox.parse.lr1.LRTable`
  (single-action map + conflicts list).
* :mod:`uplox.parse.glr.runtime` — the Tomita driver. Operates on tokens and
